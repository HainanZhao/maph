...
1. **Conjecture 5.4 could have been transcribed incorrectly.** The statement
   was checked directly against arXiv:2605.12822v1, Section 5.2. Its
   hypotheses are `r >= 2`, `k >= 1`, and positive integers `a_1,...,a_k,b`.
   Its two sufficient conditions are joined by “or,” and the second is the
   inclusive inequality
   `b <= 1 + sum_i floor(a_i/r)`. The source conjectures, rather than proves,
   necessity when `k <= 3` or `r <= 3`.

2. **The recursion could have an off-by-one spacer.** Expanding both sides
   confirms
...
3. **A sum of unimodal terms need not be unimodal.** The proof does not use
...
4. **Translation might destroy symmetry.** The translated old polynomial is
...
5. **The allocation could create a zero q-integer.** This cannot occur in the
...
6. **The divisibility disjunct might be silently omitted.** It is handled
   separately by
...
7. **The theorem might accidentally claim necessity.** The abstract,
...
8. **The named result might already have changed.** The exact primary source
