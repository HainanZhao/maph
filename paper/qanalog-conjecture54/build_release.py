#!/usr/bin/env python3
"""Build the deterministic Zenodo release for the Conjecture 5.4 proof."""
...
DOI = "10.5281/zenodo.21830407"
ROOT = Path(__file__).resolve().parents[2]
PAPER = ROOT / "paper" / "qanalog-conjecture54"
ARCHIVE_FILES = {
    ...
    "proof/qanalog_conjecture54_sufficiency.py": (
        ROOT / "proof" / "qanalog_conjecture54_sufficiency.py"
    ),
    "proof/qanalog_conjecture54_sufficiency.md": (
        ROOT / "proof" / "qanalog_conjecture54_sufficiency.md"
    ),
}
...
def main() -> None:
    ...
    manifest = {
        "claim": (
            "Conjecture 5.4 sufficient direction for all k >= 1 and r >= 2"
        ),
        "doi": DOI,
        "files": {
        ...
        "schema": "qanalog-conjecture54-release-v1",
    }
    manifest_data = (json.dumps(manifest, indent=2, sort_keys=True) + "\n").encode()

    pdf_target = args.output / "01-aligned-center-q-integer-paper.pdf"
    tex_target = args.output / "02-aligned-center-q-integer-source.tex"
    zip_target = args.output / "03-aligned-center-q-integer-replay.zip"
