...
obligations.  The current v5 transport manifest records 232 eligible rows
and zero completed member transports.
...
rows are added.  A scope manifest records each member, its canonical
closure representative, and its frozen finite ideal, but assigns every
member the status \texttt{UNSTARTED\_NO\_CASE\_LEVEL\_PACKET\_CLAIM}.
...
The currently overlapping base discriminants are recorded, but no
object-level equality is inferred from a common base field.  Likewise,
Dummit--Sands--Tangedal provide theorem-class antecedents for
quadratic-character phenomena; this paper makes no priority assertion
from that overlap.  The detailed comparison table remains an appendix
artifact rather than an input to any census count.
...
in \url{artifacts/census-h-taxonomy-v1.json}; and Engine-B transport
scope is in \url{artifacts/engine-b-transport-manifest-v5.json}.
The last artifact's zero completed transports is intentional
claim-boundary data.  The project checksum manifest and full test suite
