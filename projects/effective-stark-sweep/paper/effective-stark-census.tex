...
\date{1 August 2026\\\small\url{https://doi.org/10.5281/zenodo.21729947}}
...
criterion, and twelve named higher-order member transports.  The
...
rows.  Its successor ledger records twelve completed noncanonical member
transports; the other 220 remain open.
...
when one such value is $1$, proving the equivalence of (3) and (4).
The class numbers, roots-of-unity quotient, index, and oriented
$\log|\tau_1u_\chi|$ in \eqref{eq:quadratic-product} are nonzero, so the
explicit quadratic formula gives (2) if and only if (3).  Fourier
inversion gives (2) implies (1).  Conversely, (1) gives
$Z'_{\mathfrak m}(0,A)=0$ for every $A$; character orthogonality applied
to the exact inverse transform recovers (2).
\end{proof}
...
record route eligibility, not packet identities.
...
exactly the twelve noncanonical members in Table~\ref{tab:transport}.
...
\begin{table}[ht]
\centering\small
\caption{All proved noncanonical Engine-B member transports.}
\label{tab:transport}
...
remaining 220 members retain status
...
the H-taxonomy matrix.  The deterministic paper-and-replay package is
archived at \url{https://doi.org/10.5281/zenodo.21729947}.  The companion
paper proves the uniform
...
\url{https://doi.org/10.5281/zenodo.21729947}.
\end{sloppypar}
