...
The preregistered worked imprimitive row is RQ-000013 over
\(\mathbb Q(\sqrt2)\), with modulus HNF
\(\left(\begin{smallmatrix}14&6\\0&2\end{smallmatrix}\right)\).
Its unique supported quadratic character has primitive conductor HNF
\(\left(\begin{smallmatrix}7&3\\0&1\end{smallmatrix}\right)\);
the omitted norm-two prime has character value \(-1\), so
\(E_\chi=2\).  The exact free-unit norm map is \((1,-1)\), its
primitive kernel is generated by \((1,1)^T\), and the embedded base
unit has vector \((1,-1)^T\), giving \(I_\chi=2\).  For the oriented
relative unit
\[
u=\frac{t^2-2t+3}{4},\qquad t^4+2t^2-7=0,
\]
exact root isolation gives \(9/5<u<19/10\), while the nontrivial Artin
element sends \(u\) to \(u^{-1}\).  Hence the uniform theorem and
Fourier inversion prove the labelled identity
\[
X_{[0]}=u^2,\qquad X_{[1]}=u^{-2}.
\]
This row is `PROVED` by
`artifacts/rq000013-engine-a-imprimitive-certificate-v1.json`.
The archived `bnrL1` point-value residual is an `OBSERVED` quarantined
cross-check and is not part of the proof.
