\\ Exact counterexample screen for one selected modulus beyond the frozen range.
...
run_screen() =
{
  ...
  if(#supported != 4,
    print("FOUR_SUPPORT_ALL_ZERO=0");
    print("Q_FOUR_SUPPORT_COUNTEREXAMPLE_SCREEN_VERIFIED=1");
    return());
  ...
  for(index = 1, #supported,
    ...
    my(primitive_chi = primitive_character(
      Vec(primitive_ray.cyc), conductor_data[3]));
    my(primitive_cyc = Vec(primitive_ray.cyc));
    my(factors = idealfactor(K, finite_ideal));
    my(this_zero = 0, split_deleted = 0, removed = 0);
    for(row = 1, matsize(factors)[1],
      my(prime_ideal = factors[row, 1]);
      if(idealval(K, primitive_modulus[1], prime_ideal) == 0,
        my(prime_log = Vec(bnrisprincipal(
          primitive_ray, prime_ideal, 0)));
        my(value_log = character_on(
          primitive_chi, prime_log, primitive_cyc));
        my(value_denominator = denominator(value_log));
        if(value_denominator != 1 && value_denominator != 2,
          error("nonquadratic local value"));
        removed++;
        if(value_denominator == 1,
          this_zero = 1;
          split_deleted++));
    );
    ...
    print("CHARACTER_", index, "_SPLIT_DELETED_COUNT=", split_deleted);
    print("CHARACTER_", index, "_ZERO_EULER=", this_zero);
  );
  print("ZERO_EULER_CHARACTER_COUNT=", zero_count);
  print("FOUR_SUPPORT_ALL_ZERO=", zero_count == 4);
  print("Q_FOUR_SUPPORT_COUNTEREXAMPLE_SCREEN_VERIFIED=1");
};
