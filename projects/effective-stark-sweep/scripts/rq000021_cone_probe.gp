\\ Exact cone data for RQ-000021.
default(parisizemax,2000000000);
run_case() =
{
  my(Kpol=y^2-2,K=bnfinit(Kpol,1),f=[7,0;0,7]);
  my(r=bnrinit(K,[f,[1,0]],1),eps=Mod(3+2*y,Kpol),ord=0);
  ...
  for(a=0,5,my(A=idealpow(K,r.gen[1],a),b=idealdiv(K,f,A));
    my(best=10^99,el=0,co=0,ma=0);
    for(u=-120,120,for(v=-120,120,if(u||v,my(c=b*[u,v]~,z=nfbasistoalg(K,c),em=nfeltembed(K,z));
      if(em[1]>0&&em[2]>0,my(c2=nfalgtobasis(K,z*eps),m=matconcat([c,c2]),ix=abs(matdet(m)/matdet(b)));
