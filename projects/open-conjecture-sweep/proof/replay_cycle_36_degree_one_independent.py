#!/usr/bin/env python3
"""Independent direct-set replay of Cycle 36's degree-one functional."""
...
SOURCE = ROOT / "discovery/out/cycle36-degree-one-pseudoexpectation/result.json"
OUTPUT = ROOT / "discovery/out/cycle36-degree-one-pseudoexpectation/independent-replay.json"
...
def main() -> None:
    ...
    if len(representatives) != 1394:
        raise AssertionError("predicate representatives")
    option_covered = []
    ...
    for point in representatives:
        ...
        degree_zero_value = math.prod(dots)
        if degree_zero_value:
            degree_zero_nonzero.append(point)
        ordinary = sum(dot == 0 for dot in dots)
        strong = sum(all(normal[option] * bit == 0 for option, bit in enumerate(pattern)) for normal, pattern in zip(normals, patterns))
        ordinary_counts.append(ordinary)
        strong_counts.append(strong)
        if not (ordinary >= 2 or strong >= 1):
            raise AssertionError(f"ordinary-or-strong compression {point}")
        for coordinate, pattern in enumerate(patterns):
            other = math.prod(dot for other_coordinate, dot in enumerate(dots) if other_coordinate != coordinate)
            for option, bit in enumerate(pattern):
                raw_degree_one += 1
                if bit == 0:
                    automatic += 1
                value = normals[coordinate][option] * bit * other
                if value:
                    degree_one_nonzero.append((point, coordinate, option, value))
    if raw_degree_one != 221646 or automatic != 31768:
        raise AssertionError("raw generator census")
    if degree_zero_nonzero or degree_one_nonzero:
        raise AssertionError("nonzero raw generator")

    primary_class = source["p199"]["predicate_classification"]
    if histogram(ordinary_counts) != primary_class["ordinary_kill_histogram"]:
        raise AssertionError("ordinary histogram")
    if histogram(strong_counts) != primary_class["strong_kill_histogram"]:
        raise AssertionError("strong histogram")
    if sum(value > 0 for value in strong_counts) != primary_class["predicates_with_strong_kill"]:
        raise AssertionError("strong predicate count")
    ...
    result = {
        ...
        "p199": {
            ...
            "automatic_degree_one_generators": automatic,
            ...
            "ordinary_or_strong_equivalence_pass": True,
    ...
    print(json.dumps({"status": "PASS", "degree_zero_nonzero": 0, "degree_one_nonzero": 0, "raw_degree_one": raw_degree_one, "local_masses": masses, "max_abs_coefficient": result["p199"]["maximum_absolute_local_coefficient"]}, sort_keys=True))
