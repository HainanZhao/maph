...
int main(int ac,char**av){if(ac!=2)return 2;std::vector<std::array<int,3>>q;std::array<int,3>a{0,1,2};do q.push_back(a);while(std::next_permutation(a.begin(),a.end()));std::map<std::array<int,3>,int>ix;for(int i=0;i<6;++i)ix[q[i]]=i;int m[6][6],iv[6],cl[6];for(int i=0;i<6;++i)for(int j=0;j<6;++j){std::array<int,3>z;for(int k=0;k<3;++k)z[k]=q[i][q[j][k]];m[i][j]=ix[z];}for(int i=0;i<6;++i)for(int j=0;j<6;++j)if(m[i][j]==0&&m[j][i]==0)iv[i]=j;for(int z=0;z<6;++z){int v=z,n=1;while(v){v=m[v][z];++n;}cl[z]=n==1?0:n==2?1:2;}P A=var(0),B=var(1),C=var(2),T=var(3),one;one[0]=1;P total;
for(int x1=0;x1<6;++x1)for(int x2=0;x2<6;++x2)for(int x3=0;x3<6;++x3)for(int x4=0;x4<6;++x4){int x[5]{0,x1,x2,x3,x4};P product=one;for(auto nb:N){P sum;for(int y=0;y<6;++y){P z=one;for(int i:nb){int h=m[iv[x[i]]][y];P f=cl[h]==0?A:cl[h]==1?B:C;if(h==3)f=plus(f,var(3,-1));if(h==4)f=plus(f,T);z=mul(z,f);}sum=plus(sum,z);}product=mul(product,sum);}total=plus(total,product);}std::filesystem::create_directories(av[1]);std::ofstream o(std::string(av[1])+"/s3-c3-polynomial.tsv");o<<"a\tb\tc\tt\tcoefficient\n";int n=0;for(auto[k,v]:total){int e[4];for(int i=0;i<4;++i)e[i]=k>>(4*i)&15;if(e[3]&&v){ // subtract t=0 automatically
o<<e[0]<<'\t'<<e[1]<<'\t'<<e[2]<<'\t'<<e[3]<<'\t'<<v<<'\n';++n;}}
std::ofstream j(std::string(av[1])+"/symbolic-summary.json");j<<"{\"status\":\"PASS\",\"nonconstant_terms\":"<<n<<"}\n";}
