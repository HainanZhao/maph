...
int main(int argc, char** argv) try {
  ...
  fs::path work_root = "discovery/out/cycle6-sample-work";
  for (int i = 1; i < argc; ++i) {
    ...
    else if (arg == "--stop-depth") stop_depth = std::stoi(value());
    else if (arg == "--sample-count") sample_count = std::stoull(value());
    else {
  ...
  if (k < 2 || k > MAX_K || !is_prime(p) || (p - 1) / 2 > 127 || threads != 3 ||
      ...
      output_path.empty() || (stop_depth >= 0 &&
      (stop_depth > k || sample_count < 1))) {
  ...
  for (int depth = 0; depth <= k && !stopped.load(std::memory_order_relaxed); ++depth) {
    if (depth == stop_depth) {
      ...
      for (int partition = 0; partition < PARTITIONS; ++partition) {
        fs::path path = level_path(work_root, depth, partition);
        require_aligned(path);
        if (fs::exists(path)) bytes += fs::file_size(path);
      }
      ...
      for (int partition = 0; partition < PARTITIONS; ++partition) {
        ...
        std::uint64_t part_bytes = fs::file_size(path);
        std::ifstream input(path, std::ios::binary);
        input.read(reinterpret_cast<char*>(states.data() + offset),
                   static_cast<std::streamsize>(part_bytes));
        if (!input) throw std::runtime_error("short sample-state read");
        offset += static_cast<std::size_t>(part_bytes / RECORD_BYTES);
      }
      std::sort(states.begin(), states.end());
      states.erase(std::unique(states.begin(), states.end()), states.end());
      std::ofstream output(output_path);
      sampled_rows = std::min<std::uint64_t>(sample_count, states.size());
      for (std::uint64_t row = 0; row < sampled_rows; ++row) {
        State const& state = states[static_cast<std::size_t>(row)];
        for (int index = 0; index < depth; ++index) {
          output << (index == 0 ? "" : " ")
                 << static_cast<int>(state.value[static_cast<std::size_t>(index)]);
        }
        output << '\n';
      }
      if (!output) throw std::runtime_error("sample write failed");
      completed_depth = depth;
      std::cerr << "sample_depth=" << depth << " total_states=" << states.size()
  ...
  bool complete = !stopped.load(std::memory_order_relaxed) &&
      completed_depth == (stop_depth >= 0 ? stop_depth : k);
