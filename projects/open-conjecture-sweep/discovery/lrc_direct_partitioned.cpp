...
struct PackingCut {
  ...
  explicit PackingCut(Engine const& source)
      : engine(source), incompatible(static_cast<std::size_t>(source.h)) {
    std::vector<char> difference(static_cast<std::size_t>(engine.h), 0);
    std::vector<int> bad;
    for (int time = 0; time < engine.h; ++time) {
      if (engine.covers[0][static_cast<std::size_t>(time / 64)] & (1ULL << (time % 64))) {
        bad.push_back(time);
      }
    }
    for (int left : bad) {
      for (int right : bad) {
        difference[static_cast<std::size_t>((left - right + engine.h) % engine.h)] = 1;
      }
    }
    for (int left = 0; left < engine.h; ++left) {
      for (int right = 0; right < engine.h; ++right) {
        if (left != right &&
            !difference[static_cast<std::size_t>((left - right + engine.h) % engine.h)]) {
          incompatible[static_cast<std::size_t>(left)][static_cast<std::size_t>(right / 64)] |=
              1ULL << (right % 64);
        }
      }
    }
  ...
  static int first(Mask const& mask, int words) {
  ...
  static void clear(Mask& mask, int vertex) {
    mask[static_cast<std::size_t>(vertex / 64)] &= ~(1ULL << (vertex % 64));
  }

  bool search(Mask candidates, int need, std::array<int, 6>& witness, int chosen) const {
    if (need == 0) return true;
    int available = popcount(candidates, engine.words);
    if (available < need) return false;
    while (available >= need) {
      int vertex = first(candidates, engine.words);
      if (vertex < 0) return false;
      clear(candidates, vertex);
      --available;
      witness[static_cast<std::size_t>(chosen)] = vertex;
      Mask next{};
      for (int word = 0; word < engine.words; ++word) {
        next[static_cast<std::size_t>(word)] =
            candidates[static_cast<std::size_t>(word)] &
            incompatible[static_cast<std::size_t>(vertex)][static_cast<std::size_t>(word)];
      }
      if (search(next, need - 1, witness, chosen + 1)) return true;
    }
    return false;
  }

  bool has_validated_witness(Mask const& covered, int remaining) const {
    if (remaining < 0 || remaining > 5) return false;
    Mask uncovered{};
    for (int word = 0; word < engine.words; ++word) {
      uncovered[static_cast<std::size_t>(word)] =
          engine.full[static_cast<std::size_t>(word)] &
          ~covered[static_cast<std::size_t>(word)];
    }
    std::array<int, 6> witness{};
    int count = remaining + 1;
    if (!search(uncovered, count, witness, 0)) return false;

    for (int i = 0; i < count; ++i) {
      int left = witness[static_cast<std::size_t>(i)];
      if (left < 0 || left >= engine.h ||
          (covered[static_cast<std::size_t>(left / 64)] & (1ULL << (left % 64)))) {
        throw std::runtime_error("invalid packing witness vertex");
      }
      for (int j = 0; j < i; ++j) {
        int right = witness[static_cast<std::size_t>(j)];
        if ((incompatible[static_cast<std::size_t>(left)]
                         [static_cast<std::size_t>(right / 64)] &
             (1ULL << (right % 64))) == 0) {
          throw std::runtime_error("invalid packing witness edge");
        }
      }
    }
    return true;
...
Mask direct_subtract(Mask mask, Mask const& cover, int words) {
  for (int word = 0; word < words; ++word) {
    mask[static_cast<std::size_t>(word)] &= ~cover[static_cast<std::size_t>(word)];
  }
  return mask;
}

bool direct_feasible(Engine const& engine, Mask uncovered, int remaining,
  ...
  int target = engine.first_uncovered(direct_subtract(engine.full, uncovered, engine.words));
  std::vector<std::pair<int, int>> choices;
  for (int center : engine.covering_centers[static_cast<std::size_t>(target)]) {
    Mask next = direct_subtract(uncovered, engine.covers[static_cast<std::size_t>(center)], engine.words);
    choices.emplace_back(popcount(next, engine.words), center);
  }
  std::sort(choices.begin(), choices.end());
  for (auto [ignored, center] : choices) {
    (void)ignored;
    if (direct_feasible(engine,
                        direct_subtract(uncovered, engine.covers[static_cast<std::size_t>(center)], engine.words),
                        remaining - 1, memo)) {
      memo.emplace(std::move(key), true);
      return true;
    }
  }
  memo.emplace(std::move(key), false);
  return false;
}
...
int main(int argc, char** argv) try {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (arg == "--packing-table") packing_table_path = value();
    else {
  ...
  if (k < 2 || k > MAX_K || !is_prime(p) || (p - 1) / 2 > 127 || threads != 3 ||
      ...
      (output_path.empty() && packing_table_path.empty())) {
    std::cerr << "invalid arguments\n";
    return 2;
  }
  Engine engine(k, p);
  PackingCut packing(engine);
  if (!packing_table_path.empty()) {
    if (engine.h > 20) throw std::runtime_error("packing table is restricted to h<=20");
    std::ofstream table(packing_table_path);
    std::uint64_t subset_count = 1ULL << engine.h;
    for (std::uint64_t subset = 0; subset < subset_count; ++subset) {
      Mask covered = engine.full;
      for (int time = 0; time < engine.h; ++time) {
        if (subset & (1ULL << time)) PackingCut::clear(covered, time);
      }
      for (int remaining = 1; remaining <= 3; ++remaining) {
        table << subset << ' ' << remaining << ' '
              << packing.has_validated_witness(covered, remaining) << '\n';
      }
    }
    if (!table) throw std::runtime_error("packing table write failed");
    return 0;
  }
  ...
  std::atomic<std::uint64_t> expanded(0), edges(0), leaves(0), live_disk(0), peak_disk(0),
      packing_checks(0), packing_prunes(0), direct_checks(0), direct_prunes(0);
  ...
  for (int depth = 0; depth <= k && !stopped.load(std::memory_order_relaxed); ++depth) {
      ...
      workers.emplace_back([&, worker]() {
        ...
        while (!stopped.load(std::memory_order_relaxed)) {
          ...
          while (!stopped.load(std::memory_order_relaxed) &&
            ...
            if (remaining <= 5) {
              packing_checks.fetch_add(1, std::memory_order_relaxed);
              if (packing.has_validated_witness(covered, remaining)) {
                packing_prunes.fetch_add(1, std::memory_order_relaxed);
                continue;
              }
              direct_checks.fetch_add(1, std::memory_order_relaxed);
              std::unordered_map<std::string, bool> memo;
              Mask uncovered{};
              for (int word = 0; word < engine.words; ++word) {
                uncovered[static_cast<std::size_t>(word)] =
                    engine.full[static_cast<std::size_t>(word)] & ~covered[static_cast<std::size_t>(word)];
              }
              if (!direct_feasible(engine, uncovered, remaining, memo)) {
                direct_prunes.fetch_add(1, std::memory_order_relaxed);
                continue;
              }
            }
    ...
    std::cerr << "level=" << depth << " states=" << level_states
              ...
              << " packing_checks=" << packing_checks.load()
              << " packing_prunes=" << packing_prunes.load()
              << " direct_checks=" << direct_checks.load()
              << " direct_prunes=" << direct_prunes.load()
  ...
  std::cout << "packing_checks=" << packing_checks.load() << '\n';
  std::cout << "packing_prunes=" << packing_prunes.load() << '\n';
  std::cout << "direct_checks=" << direct_checks.load() << '\n';
  std::cout << "direct_prunes=" << direct_prunes.load() << '\n';
