# PROGRAM: Beyond Guth--Maynard Zero Density

## Purpose, boundary, and status

- Project cap: Cycle 500. Cycle 184 is the latest sealed cycle, so the
  remaining authorized cycle labels are 185--500.
...
- `STRATEGIC_DECISION`: the standing session mentor recommended this same
  ...
  adopted. The project itself is not abandoned: 316 authorized cycle labels
...
`PROGRAM.md` is strategic state, not a laboratory notebook. The canonical
...
- `PROVED`: Cycle 179 rules out the entire exact-rational, beta-zero
  cross-label class by gcd/Bézout compression to one rational-base tower of
...
- `PROVED`: Cycle 181 restores the fixed translate `beta` and exactifies a
  new four-row common-intercept invariant. Every stable rectangle lies in one
  of at most `H` rational-intercept packets; a critical stable population
  supplies one full labelled packet of `X^(21/25-o(1))` rectangles. This is a
  coefficient-preserving reduction, not a recurrence or census bound.
- `PROVED`: Cycle 182 upgrades every participating non-singleton fibre in one
  packet to a complete primitive rational-line segment. Its intercept
...
- `PROVED`: Cycle 184 identifies `gcd(u,w)|F` exactly as an LCM resonance
  generated by the two existing orbit errors. A scale-matched two-label,
  actual-positive-exponential, nonrational deformation has full fibres below
  the seed scale and only `X^(11/25-o(1))` rectangles in one box. This banks
  `LCM_RESONANCE_REDUNDANCY`, not a critical-box saturation or impossibility
  theorem.
...
published reconstruction P0 [complete]
  ...
  `-- E13 actual positive-exponential census [PRIMARY THROUGH C216]
        ...
        +-- stable determinant shells --> intercept packets --> primitive lines
            --> populated primitive ray boxes --> LCM-only redundancy [complete]
            --> three-label/shared-fibre or cross-box entropy [active]
...
|---|---|---|
| E13 / Cycle 63 | `LCM_REDUNDANCY_BANKED_POPULATION_COMPATIBILITY_ACTIVE` | Use three-label/shared-fibre compatibility or cross-box coefficient entropy to prove a coefficient-preserving bound for a populated primitive-ray box sufficient for a strict direct triple margin, force a seeded recurrence, or construct a genuine critical nonrational box saturator. The individual heavy-fibre route is proved but is not yet an aggregate E7/E9 recurrence bound. Raw pairs, exact-rational towers, low-product rectangles, a packet/line/box partition, ray rigidity, or an LCM-only identity without a population estimate are disallowed as progress. |
...
|---|---|---|---|
| Improved density / intervals | `PROVED` complete baseline ledger, explicit strict exponent targets, heavy-fibre extraction, intercept packets, primitive-line rigidity, a populated primitive-ray box, and LCM redundancy | Any fixed-power population-compatible box census saving or seeded recurrence surviving reinsertion | Continue only through the Cycle-63 sprint; intervals remain downstream. |
| Larger saturation theorem | `PROVED` `EO-LF4` exponent-20 sharpness, `lambda*Xi` reduction, C177 pair saturator, C178 heavy-fibre extraction, C179 rational no-go, C180 shell reduction, C181 intercept exactification, C182 line rigidity, C183 populated ray box, and C184 LCM redundancy | Exhaustiveness for a materially larger coefficient-sensitive class or a genuinely critical nonrational primitive-ray box saturator | Test population compatibility, not another two-orbit identity. |
...
|---|---|---|
| 177--216 | Cycles 177--184 saturated raw pairs, extracted heavy fibres, excluded exact-rational towers and low products, reduced light critical mass to populated primitive common-intercept ray boxes, and banked the redundancy of the two-orbit LCM identity. Test three-label/shared-fibre compatibility and cross-box coefficient entropy for a population-sensitive estimate, seeded recurrence, or genuinely critical nonrational saturator. | At Cycle 216 retain E13 only if a mechanism has a quantified path to fixed-power triple margin, aggregate recurrence control, or a materially larger actual primitive-ray-box saturation theorem. No new terminal classifier or LCM-only identity counts as progress. |
...
1. Can three labels with shared fibres impose a coefficient-preserving
   compatibility relation not generated by pairwise orbit errors, and force a
   depth-`X^(6/25)` seed or a strict populated-box saving?
2. Can cross-box coefficient entropy bound the number of compatible varying
   LCM resonances, or does the actual positive exponential admit a genuinely
   critical **nonrational** primitive-ray-box saturator?
...
Cycle 184 is sealed at
`artifacts/cycle-184-ray-box-determinant-orbit-v1.json`. The mentor required
and approved the narrow boundary: the LCM relation is generated by the two
orbit errors, while the nonrational deformation is subcritical and establishes
neither saturation nor a critical-box impossibility theorem. Next authorized
action: preregister one substantive Cycle-185 population-compatibility engine,
choosing either three-label/shared-fibre compatibility or cross-box coefficient
entropy. It must retain the complete C183 state and add a relation not
algebraically generated by two orbit errors; its advance is a quantified
populated-box bound, forced seeded recurrence, or genuine critical nonrational
saturator—not an LCM identity, a two-label deformation, a box partition,
scalar count, raw pair, exact-rational tower, low-product rectangle, or
terminal classifier. Cycles 165--176 remain reference constraints, not the
active derivation target.
...
cat projects/guth-maynard-zero-density/PROGRAM.md
