#!/usr/bin/env python3
"""Checks the literal-test correction for phase-lattice Base-saturation v1."""
...
V1_ARTIFACT = PROJECT / "artifacts/cycle-7-crr-phase-lattice-base-saturation-v1.json"
V1_BUILDER = PROJECT / "proof/build_cycle_7_crr_phase_lattice_base_saturation_v1.py"
V1_DOCUMENT = PROJECT / "docs/cycle-7-crr-phase-lattice-base-saturation-v1.md"
V1_TEST = PROJECT / "tests/test_cycle_7_crr_phase_lattice_base_saturation_v1.py"
ARTIFACT = PROJECT / "artifacts/cycle-7-crr-phase-lattice-base-saturation-v1-test-correction.json"
SCRIPT = PROJECT / "proof/build_cycle_7_crr_phase_lattice_base_saturation_v1_test_correction.py"


def load_builder():
    spec = importlib.util.spec_from_file_location("crr_phase_lattice_base_saturation_test_correction_builder", SCRIPT)
...
class PhaseLatticeBaseSaturationV1TestCorrectionTests(unittest.TestCase):
    def test_preserves_v1_and_checks_exact_sealed_phrase(self) -> None:
        correction = json.loads(ARTIFACT.read_text(encoding="utf-8"))
        v1 = json.loads(V1_ARTIFACT.read_text(encoding="utf-8"))
        self.assertEqual(correction["artifact_id"], "cycle-7-crr-phase-lattice-base-saturation-v1-test-correction")
        ...
        self.assertEqual(v1["artifact_id"], "cycle-7-crr-phase-lattice-base-saturation-v1")
        original_test = V1_TEST.read_text(encoding="utf-8")
        document = V1_DOCUMENT.read_text(encoding="utf-8")
        self.assertIn('self.assertIn("Exact rational aliases offer only constant factors", document)', original_test)
        self.assertIn("exact aliases can therefore provide at most constant factors", document)
    ...
    def test_historical_failure_is_contained_and_replay_passes(self) -> None:
        ...
        self.assertIn("Exact rational aliases offer only constant factors", historical.stdout + historical.stderr)
