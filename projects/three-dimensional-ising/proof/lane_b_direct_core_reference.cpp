// Independent spin-slice reference for the direct polynomial-core firewall.
...
int main(){
    int n,w,dimension,edge_count,evaluation;
    if(!(std::cin>>n>>w>>dimension>>edge_count>>evaluation))return 2;
    ...
    auto base_weight=[evaluation](int hash)->u64{
        return 2+(static_cast<u64>(hash+1)*104729ULL
                  +static_cast<u64>(evaluation)*13007ULL)%(P-3);
    };
    ...
    for(int layer=0;layer<n;++layer){
        ...
        if(__builtin_popcount(residual_support[layer])>8){
            std::cerr<<"too many nonexact modes\n";return 4;
        }
    }
        ...
        for(int local=0;local<variants;++local){
            ...
            for(int spin=0;spin<states;++spin){
                ...
                for(const auto&e:transverse[layer]){
                    ...
                    u64 weight=base_weight(hash);
                    if(__builtin_popcount(character&e.label)&1)weight=P-weight;
    ...
    for(int character=0;character<characters;++character){
        ...
        auto parameters=[&](int layer)->std::pair<int,const u64*>{
            int gauge=0;
            for(int bit=0;bit<dimension;++bit)
                if((character&(1U<<bit))&&exact_gauge[layer][bit]>=0)
                    gauge^=exact_gauge[layer][bit];
        ...
        auto [first_gauge,first_diagonal]=parameters(0);
        ...
        for(int layer=0;layer+1<n;++layer){
            ...
            auto [gauge,diagonal]=parameters(layer+1);
            for(int spin=0;spin<states;++spin)state[spin]=mulmod(transformed[spin],diagonal[spin^gauge]);
        }
        u64 total=0;for(u64 x:state)total=addmod(total,x);
        answer[character]=addmod(total,total);
    }
