#!/usr/bin/env python3
"""Seal the symbolic encoder boundary-incidence proof and rotation firewall."""
...
OUTPUT = ROOT / "artifacts/cycle-12-b12-encoder-incidence-proof-v1.json"
HASHES: dict[str, tuple[str, str]] = {
    ...
    "manuscript": ("paper/canonical-spin-structure-compression/main.tex", "77ef57eaa1eb2b3402aa3b844915d0268df03128e67b288e6b4fc9589c474de4"),
...
def payload() -> dict[str, object]:
    ...
    for row in rows:
        if any(item["unclassified_count"] for item in row["normal"]):
            raise RuntimeError("normal incidence gained an unclassified edge")
        if not all(item["is_dual_component"] for item in row["normal"]):
            raise RuntimeError("a normal island ceased to be a dual component")
        if row["opposite"].get("unclassified_count", 0):
    ...
    return {
        "artifact_id": "cycle-12-b12-encoder-incidence-proof-v1",
        "author": "Hainan Zhao",
        "budget_ordinal": "B12",
        "cycle": 12,
        "status": "SEALED",
        "epistemic_status": "PROVED_WITH_EXACT_COMBINATORIAL_FIREWALL",
        "record_type": "LANE_B_ENCODER_INCIDENCE_PROOF",
        "outcome": (
        ...
        "runtime": check_runtime("cycle-12-encoder-incidence-proof"),
        "sealer": {
            "path": "proof/build_cycle12_encoder_incidence_proof.py",
        ...
        "replay": {
            ...
            "artifact_check": "python3 proof/build_cycle12_encoder_incidence_proof.py --check",
        },
    }
