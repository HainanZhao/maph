...
\hypersetup{
  pdftitle={Separator compression of the complete spin-structure family for cubic Ising strips},
...
We then prove that this bound is generically sharp for independent nonuniform
weights for every \(w\geq3\); \(n=11\) is uniformly sufficient.  Thus surface
...
Concretely, for a right partial-chain space \(P_R\) with trace
\(\tau_R(c)=\partial c|_S\), there are \(s\in C^0(G_R;\Ftwo)\) and a linear
...
\(b_i\)-character on the exposed half is a function \(\rho_i(m)\).  The
polarization term \(\lambda_{a_i}\lambda_{b_i}\) may then be assigned to the
\(a_i\) side conditional on this known trace.
\end{description}
\end{definition}
...
At an internal handle cut, H3 turns the remaining \(b_i\)-phase into a
diagonal function of the same \(m\).  The conditional \(a_ib_i\) term stays
in the left factor, so the intermediate space does not grow.  Without H3,
...
cap disks.  This gives \cref{eq:genus} by Euler characteristic.  When \(n,w\)
are even all faces are quadrilaterals, so the bipartite girth-four Euler lower
bound is attained.
\end{proof}
...
In any pinned homology basis with intersection matrix \(\Omega\), solve
\(A^T\Omega B=I\), choosing the lexicographically least solutions with free
bits zero.  Put \(C=B^T\Omega B\), and let \(U\) be the strictly upper
triangular matrix with the same upper triangle as \(C\).  Replacing
\(B\) by \(B+AU\) gives
\[
 A^T\Omega B=I,\qquad A^T\Omega A=B^T\Omega B=0.
\]
This construction retains old slab co-cores when a terminal slice is added.
The geometric longitude dual to each co-core lies in its local slab collar,
which supplies the claimed filtration adaptation.
\end{proof}
...
side of the co-core, so its value is \(\langle s_i,m\rangle\).  It is therefore
frontier known, which is H3.  This is the point that prevents a spurious
...
the corresponding integer-polynomial minor is not identically zero.
\end{proof}
