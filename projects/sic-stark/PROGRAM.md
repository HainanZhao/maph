...
- Current state: Cycle 198 proves that the meromorphic continuation of the
  ...
  character-valued cross form is not fusion or coinvariant descent. Cycle 214
...
- `PROVED`: Cycle 191 restricts the published beta Fourier character to its
  two canonical level-six central-character blocks. All 900 frozen
  characteristic/alias rows force `epsilon=a mod 2` and recover `(a,b)`;
  however each alias step acts by `diag_j((-1)^j)`, which the block-scalar
...
- `PROVED`: Cycle 207 retains every elementary C198 target square and proves
  all 25 four-factor products have mismatched unoriented recurrence-class
  signatures. Standard factorwise `T1`, `T2`, and normalized-reflection
...
- `PROVED`: Cycle 212 tests the two natural logarithmic lifts from the
  attracting axis: `t=s^(-1)` and `t=s`. They are equally `A6`-covariant and
  reach the opposite C211 cusps. The selected real embedding and norm-37 ray
...
- `PROVED`: Cycle 214 identifies `E=J0*S` as the unique frozen source
  candidate that exchanges the two cusp labels, preserves `Q`, and conjugates
  `A6` to `A6^-1`. AFK transformed-tuple covariance and determinant-negative
  cocycle conjugation do not supply an action on the beta-oriented packet,
...
- `PROVED`: Cycle 216 fixes the upper semicircle from `omega1` to
  `-omega1` and a moving separating functional. It separates both complete
  true-pole cones for every interior path point, but its corridor collapses
  at the endpoint, where the limiting `m=0` pole trajectories are dense on
  the limiting real contour. The explicit one-step S--S factorization,
...
- `PROVED`: Cycle 224 derives the unique minimal joint-shift cochain
  `exp(pi*i*tilde-u_-)` and proves its commutator integrability. Applied to the
  frozen `epsilon=±1` products, it makes all four raw reflection products
...
- `PROVED`: Cycle 227 gives the exact raw-parity/`576`-period/affine-sign
  normal form for the augmented transport. No generic-label scaling quotient
