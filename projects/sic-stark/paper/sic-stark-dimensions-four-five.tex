...
algebraicity theorem gives a safe exponent $5760$; labeled ray-field
...
cyclic-quartic resolvents remain in dimension eight.  The finite
...
\cite{AFK} associates convention-sensitive Shintani--Faddeev
...
\[
 ...
 {\cal D}^{(4)}_{4-p,q}{\cal D}^{(4)}_{4-r,p}
 {\cal D}^{(4)}_{4-q,r}.
\tag{D4.DS}
\]
...
\[
\begin{aligned}
 {\cal Q}_6(X)={}&X^{12}+3X^{11}-6X^{10}-16X^9+3X^8+27X^6\\
...
\[
 {\cal O}_3=\Z+3{\cal O}_{K_8}.
\]
The order-ray group and its maximal-order realization are
\[
 \Cl_{(8)\infty_2}({\cal O}_3)
 \cong
 \Cl_{(24)\infty_2}({\cal O}_{K_8})
 \cong C_4\times C_2\times C_2.
\tag{36}
\]
The change-of-order isomorphism in (36) does not identify partial zeta
...
calculation \textup{(41)} are unconditional; their identification with
the primitive analytic table is conditional on \textup{(42)}.
...
At high precision the phase quotients in (42) equal
$1+O(10^{-114})$.  This is strong evidence, not an equality proof:
(40) permits an arbitrary unit-circle phase.  The exact finite TCC
orientation sieve selects one pair among the $64$ natural algebraic
orientations, but using that selection to deduce (42) would be circular.
An oriented refinement of Roblot's theorem, or powered algebraicity for
these two convention-matched cocycle values, would make (42) and hence
dimension eight unconditional without any further finite computation.
...
8&15\text{ lower overlaps and quartic moduli}
 &2\times784\text{ minors}
 &2\text{ quartic orientations}.
\end{array}
\tag{43}
\]
Dimension six is the smaller and conceptually sharper open
Stark--Shintani identity.  Dimension eight has the larger finite packet,
but its remaining analytic obstruction is already separated into two
explicit cyclic-quartic phases.
...
still requires (33) in dimension six and the two orientations (42) in
...
certificates;
\item the dimension-six ray, lower-stratum, Frobenius-orientation, and
$225$-minor audit scripts;
\item the exact dimension-six fixed-point factorization (34);
\item the dimension-eight conductor-lowering, lower-stratum,
cyclic-quartic, Artin-label, and orientation audits; and
\item the exact two-shift dimension-eight $784$-minor audit script.
\end{itemize}
