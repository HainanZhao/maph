...
bridge to the unconditional dimension-seven and dimension-eight
identities; the present paper remains logically independent of those
higher-dimensional certificates.
\end{abstract}
...
A companion paper~\cite{ZhaoII} applies the same convention-audited bridge to
dimensions seven and eight, using Shintani height rigidity in dimension
seven and a combination of linear CM reinduction and quadratic phase
reduction in dimension eight.  Dimension six remains exceptional: its
...
that platform. The submission-specific source archive is built
deterministically and contains the files below and a self-verifying
root manifest named \texttt{ARCHIVE\_CONTENTS.sha256}. Its persistent
archive DOI will be inserted after deposit; a repository commit hash
alone is not intended to substitute for that archive.
