...
recognition is used as an input. A companion paper applies the same
bridge to the discriminant-$32$ dimension-seven stratum and both
dimension-eight strata; the present paper remains logically independent
of those higher-dimensional certificates.
\end{abstract}
...
$\infty_2(\sqrt3)=-\sqrt3$.\footnote{PARI orders real embeddings by
increasing real root.  Thus its first root of $y^2-3$ is $-\sqrt3$,
so the PARI infinite-place vector \texttt{[1,0]} denotes the place
called $\infty_2$ here.  Equivalently, in the integral generator
$\beta=2+\sqrt3$ used by the scripts, the first root of
$y^2-4y+1$ is $2-\sqrt3$.  Raw conductor outputs in the certificates
use PARI's ordering.}  The modulus throughout is
\[
...
The proofs above establish the formal TCC in dimensions four and five.
A companion paper~\cite{ZhaoII} applies the same convention-audited
bridge to the discriminant-$32$ dimension-seven stratum and to both
dimension-eight strata, using Shintani height rigidity in dimension
seven and a combination of linear CM reinduction and quadratic phase
reduction in dimension eight. The discriminant-$8$ dimension-seven
stratum is not claimed there. Dimension six remains exceptional: its
finite rank-one certificate is exact, but one oriented primitive
order-six special value is not proved by the inputs used here.
...
that platform. The deterministic submission archive contains the files
below and a self-verifying root manifest. The manifest is named
\texttt{ARCHIVE\_CONTENTS.sha256}. Its persistent DOI is
\url{https://doi.org/10.5281/zenodo.21680223}; a repository commit
alone is not a substitute for that archive.
...
\end{itemize}
SHA-256 digests for every archived proof artifact are recorded in the
archive-root manifest
\begin{center}
\texttt{ARCHIVE\_CONTENTS.sha256}.
\end{center}
...
eight: Shintani height rigidity and CM descent}, companion preprint,
2026, \url{https://doi.org/10.5281/zenodo.21681700}.
\bibitem{AFK}
