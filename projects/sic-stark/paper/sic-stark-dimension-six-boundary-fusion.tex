...
multipliers in all thirty-six cases. A subsequent Fourier-normalization
audit found that the draft does not define the map from its additive
spectral coefficients to the three logarithmic ray-class values.
Accordingly the componentwise boundary estimate and its claimed
implication to the formal shifts are retired pending such a derivation.
\end{abstract}

\paragraph{Cycle-157 status.}
This is a research draft with a known formulation gap.  The raw packets
in Conjecture~\ref{conj:estimate} omit the ordinary Fourier gauge.
Restoring that gauge changes the fused alias scalar from $-q$ to $q^2$
but still gives strong, well-conditioned growth on the tested boundary
ladder.  More fundamentally, no formula below maps the resulting 36
additive Fourier coefficients to the three ray-class logarithms
$\mathcal P_j$, including branch and finite-part choices.  Thus
BF$_6$ and the assertion BF$_6\Rightarrow$MFC$_6$ are not current
claims.  The exact downstream algebra is conditional on a future,
independently defined analytic input.
...
\begin{conjecture}[Retired quantitative boundary fusion,
BF$_6(\eta)$]
\label{conj:estimate}
...
The formerly claimed implication from
Conjecture~\ref{conj:estimate} to MFC$_6$ is withdrawn by the
cycle-157 audit: the displayed packets are ungauged, and the missing
coefficient-to-ray-logarithm map is needed before any such implication
can be stated. The retired standalone formulation is retained at
...
The obstruction is not character order alone.  The Effective-Stark
Sweep case \texttt{RQ-000190} proves unconditionally that the
order-six packet for $\mathbb Q(\sqrt7)$ and
$\mathfrak p_7\infty_2$ lies in Shintani's index-two range, with safe
exponent $4032$ and independent imaginary-base reconstructions.
Its theorem record has SHA-256
\texttt{f5f68b12163f4a884e860a92ddd2dd0757c138bb9a6fb49ec3ccc780fb3030b7}.
Dimension six remains outside that proved range because its
projective/index-two and wild-ramification structure fails, not
because its primitive character has order six.
This diagnosis is sharpened by Effective-Stark Sweep case
\texttt{RQ-002057}: over $\mathbb Q(\sqrt{57})$, an order-six packet
with a power of the ramified prime $3$ is unconditionally verified
using the exact $\langle\epsilon^3\rangle$ Shintani domain and Arb
height rigidity (certificate SHA-256
\texttt{76fbc9452a76464379e8aa816ed5f0caff31368caa1867abbff4a2f7d7551be1}).
Thus neither character order six nor ramification at $3$ by itself
explains the dimension-six wall.
The sweep's final dimension-$16$ check gives the complementary
negative result: for $\mathbb Q(\sqrt{221})$ with modulus
$(16)\infty_2$, the one-place ray group is
$C_{16}\times C_4\times C_2$, but the relevant Shintani index is
$16$, not $2$.  The case is therefore
\texttt{FRONTIER(INDEX\_GT\_2)} and does not supply a fifth
unconditional TCC dimension through the present theorem.
...
This note's reproducibility archive carries the persistent DOI
\url{https://doi.org/10.5281/zenodo.21682631}.
