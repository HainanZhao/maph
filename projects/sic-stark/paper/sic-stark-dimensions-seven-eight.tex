...
Appleby--Flammia--Kopp unconditionally in dimensions seven and eight.
In dimension seven, conductor lowering places every characteristic in
...
\tag{5}
\]
The common determinant-one stabilizer is
\[
 A_7=\begin{pmatrix}239&-140\\70&-41\end{pmatrix}.
\tag{6}
\]
...
\[
 ...
 \bigl(12f_{\mathfrak d}n(S_{\mathfrak d})\bigr)=16128.
\tag{7}
\]
...
The sixteen squared-overlap representatives satisfy a reciprocal
degree-$16$ polynomial over $\Q(\sqrt2)$ with factors of degrees
$2,2,12$.  Exact class-field comparisons identify the factors with
the two quadratic lowered fields and the one-place ray-$14$ field.
Sturm intervals label all real roots.  Rigorous Arb evaluation of the
eight independent analytic values gives a powered logarithmic error
below $9.5\cdot10^{-7}$ after multiplication by $16128$, whereas
Voutier's lower bound~\cite{Voutier1996} is greater than
$5.22\cdot10^{-5}$ for every relevant degree.  The quotient of an
analytic value by its assigned algebraic root must therefore be a
root of unity; positivity and the exact cocycle multipliers select the
displayed root.

Artin labels are fixed by split primes above $17$ and $41$.  Local
Frobenius congruences identify their exact automorphisms in the labeled
ray field over $\Q(\sqrt2)$, rather than only in an abstractly
isomorphic number field.
...
\tag{10}
\]
Proposition~\ref{prop:bridge} proves both shifts for the canonical
tuple.  The form order has one relevant proper class; unconditional
$\mathrm{GL}_2(\Z)$ covariance transports the result to every
dimension-seven admissible form.
...
In the cyclic coordinates $[8,4]$ printed by the pinned
$\Q(\sqrt{-6})$ ray computations, the reinduced characters are
\[
...
The CM conductors printed by the same audit are supported at the
displayed primes above $2,3,5$, and $\psi_b$ is ramified at every
...
\textup{(14)}.  Both fields have
$e=|\mu(E_b)|=2$.  Stark's normalized complex-place absolute value is
...
the integrality clause in this specialization makes the Stark element
a global unit, not only an $S$-unit.  Fourier transforming the displayed
partial-zeta formula gives \textup{(16)}.
\end{proof}
...
unconditional covariance calculation of the companion Paper I sends
...
Section~\ref{sec:d7} proves both shifts for dimension seven and
transports them across its form class.  Section~\ref{sec:d8} proves
both dimension-eight conductors and transports within each of the only
two possible discriminants.
\end{proof}
